...
class Ion_auth_model extends CI_Model {
     ...
     * @author Mathew
     * */
    public function register($identity, $password, $email, $telcode, $additional_data = array(), $groups = array()) {
        ...
        // Users table.
        // echo $additional_data['telcode'];exit();
        // $telcode=$additional_data['telcode'];
        $var = 1;
        $identity = str_replace($telcode, "", $identity, $var);
        $data = array(
...
//echo '<pre>'; print_r($user_data); exit;
    ...
    public function edit_trainer($ids, $data) {
        $this->db->update($this->tables['users'], $data, array('id' => $ids));
        return true;
    }
     ...
     * @author Mathew
     * */
    public function login($identity, $password, $remember = FALSE, $telcode) {
...
//		                  ->where($this->identity_column, $identity)
                ->where("$this->identity_column = '$identity' AND telcode='$telcode'")
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
    ...
    public function get_user_details($id) {
        $this->db->select('u.*,img.raw_name as raw_name,img.file_ext as file_ext,g.name as group_name,ug.group_id');
      ...
      public function clear_old_files($identity) {
        if($identity){
            $this->db->where(array('user_id' => $identity));
            return $this->db->delete('image_files');
        }
        return FALSE;
    }
