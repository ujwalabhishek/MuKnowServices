...
//        }
        ...
        $data ['authors'] = $this->Register_user_model->get_facilitator_admin();
        //echo $this->db->last_query();exit();
...
//            print_r($data ['m_category']);exit();
        ...
        //$data ['authors'] = array("1", "Admin");
        $data ['authors'] =  $this->Register_user_model->get_facilitator_admin();
		//echo "<pre>"; print_r($data ['authors']);
...
//        endif;
        ...
        // $article_type = $type; // upload file types.
...
//print_r($product_data);exit();
            ...
            $article_id = $this->db->insert_id();
             $this->Articles_model->overall_review_ratings_insert_and_update($user_id , "create_article");
            if ($article_id) {
                if ($flag == 1):
                    $i = 1;
                    foreach ($file_data as $file_data_row) {
     ...
     require_once('/var/www/resourcecoach_dev/Vimeo/src/Vimeo/Vimeo.php');
...
//            } else {
                ...
                $product_data = array(
                    ...
                    'url_link' => $file_name ? $file_name : "noimage",
                    'url_thumb' => $thumb,
                    'url_duration' => $time,
            ...
            //  echo print_r($product_data);  echo $notification; exit;
                $db_result = $this->Articles_model->insert($product_data);
                    ...
                    if($notification){
                        ...
                        { 
                            ...
                            if($article_type=='subscriber')
                            {
                                ...
                                $this->article_send_pushnotification($get_article->title,$arttyp ,$get_article->cat_id);
                                $this->article_send_ios($get_article->title,$arttyp ,$get_article->cat_id);
                            }
                     ...
                     else
                     {
                        ...
                        { 
                            ...
                            {
                              $arttyp = array('subscriber');
                              $this->article_send_pushnotification_category($get_article->title,$arttyp ,$get_article->cat_id);
                              $this->article_send_ios_category($get_article->title,$arttyp ,$get_article->cat_id);
                            }
                            elseif($article_type =='non_subscriber')
                            { 
                               $arttyp = array('subscriber','non_subscriber');
...
//echo "2nd meine:". print_r($product_data);  echo $notification; exit;
            $db_result = $this->Articles_model->insert($product_data);
...
//        $this->session->set_flashdata('err_message', 'Please Select Department.');
    ...
    public function article_send_pushnotification($titles='',$article_type ,$cat_id) {
        $title = "New Article $titles has been added.";
        $response = "New Article $titles has been added.";
        ...
        if (isset($androidresult) && count($androidresult)):
            foreach ($androidresult as $row) {
             if($this->Articles_model->get_user_notification_status($row->user_id, 'article')){
                $this->gcm->addRecepient($row->gcm_regid);
             }
              else if($this->Articles_model->check_article_category_status($row->user_id, $cat_id)){
                  $this->gcm->addRecepient($row->gcm_regid);
              }
    ...
    function article_send_ios($title2='',$article_type ,$cat_id) {
         $this->db->select('gcm_users.*');
        $this->db->from('gcm_users');
        $this->db->join('users', 'users.id = gcm_users.user_id','left');
        ...
        $message1 = "New Article $title2 has been added.";
        // $rr=$this->apn->setData($title);
        if (isset($ios_results) && count($ios_results)):
            foreach ($ios_results as $row) {
    ...
    function article_sendnotification_ios($id,$message,$title) {
     ...
     public function article_send_pushnotification_category($titles='',$article_type ,$cat_id) {
        $title = "New Article $titles has been added.";
        $response = "New Article $titles has been added.";
        ...
        if (isset($androidresult) && count($androidresult)):
            foreach ($androidresult as $row) {
             if($this->Articles_model->check_article_category_status($row->user_id, $cat_id)){
                $this->gcm->addRecepient($row->gcm_regid);
             }
            }
            //$this->gcm->setMessage($message.date('d.m.Y H:s:i'));
    ...
    function article_send_ios_category($title2='',$article_type ,$cat_id) {
         $this->db->select('gcm_users.*');
        $this->db->from('gcm_users');
        $this->db->join('users', 'users.id = gcm_users.user_id','left');
        $this->db->where('gcm_users.type','ios');
    ...
    function delete_mini()
    {
       ...
       public function view_article_scores($id='') {
     
         if ($this->session->userdata('session_data'))
            $data = $this->session->userdata('session_data');
         
        $data['article_scores'] = $this->Articles_model->get_article_user_scores($id);    
       // echo '<pre>'; print_r($data['user_comments']);exit;
          $this->load->view('admin/dashboard_article_scores', $data);
     ...
     public function leader_boards()
     {
         ...
         $leader_dashboard = $this->Articles_model->get_leader_dashboard_details();
         //echo "<pre>"; print_r($leader_dashboard); exit;
         if(!empty($leader_dashboard))
          {
                    $key = 1;
                   // $leaders_data = array();
                    foreach($leader_dashboard as $key => $values)
                    {
                        if($key < 3)
                        {
                            $leaders_data['first_three'][] =  array(
                                 "username" => $values->username ,
                                  "total_score" => $values->total_score ,
                                 "user_image" => base_url().'assets/uploads/profile_image/'.$values->raw_name.$values->file_ext,
                                );
                        }
                        else
                        {
                            $leaders_data['last'][] =  array(
                                 "username" => $values->username ,
                                  "total_score" => $values->total_score ,
                                 "user_image" => base_url().'assets/uploads/profile_image/'.$values->raw_name.$values->file_ext,
                                );
                        }
                        //echo "<pre>"; print_r($leaders_data);
                    }
                  //echo "<pre>"; print_r($leaders_data);
                $data['leaders_data'] = $leaders_data;
         }         
         $this->load->view('admin/leader_dashboard', $data);
     }
}
